...
#ifndef _even_parity_checker_bit_2_4_core_h_
#define _even_parity_checker_bit_2_4_core_h_

#include <systemc.h>
#include "properties.h"


class even_parity_checker_bit_2_4_core : public sc_module 
{
    public: 
       even_parity_checker_bit_2_4_core (sc_core::sc_module_name nm, const xsc::common_cpp::properties& props)
            : sc_module(nm)
              , Din    ( "Din" )
              , Dout   ( "Dout" )
    {
        SC_HAS_PROCESS(even_parity_checker_bit_2_4_core);
        SC_METHOD(get_slice_value);
        sensitive << Din;

    }

    virtual ~even_parity_checker_bit_2_4_core() = default;
