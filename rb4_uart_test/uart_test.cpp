...
/*void stream_test_data(Uart& uart) {
    ...
    constexpr int frequency = 1000; // 10 kHz
    constexpr auto interval = microseconds(1000000 / frequency);

    uint16_t counter = 0;
    uint8_t buffer[2];

    while (true) {
        buffer[0] = static_cast<uint8_t>(counter & 0xFF);
        buffer[1] = static_cast<uint8_t>((counter >> 8) & 0xFF);
...
void stream_test_data(Uart& uart) {
    ...
    constexpr int frequency = 5500; // 1 kHz
    constexpr auto interval = microseconds(1000000 / frequency);
    constexpr auto duration = seconds(10); // run for 10 seconds
...
void stream_test_data(Uart& uart) {
    ...
    constexpr int frequency = 1000; // 1 kHz
    constexpr auto interval = microseconds(1000000 / frequency);
    constexpr auto duration = seconds(10); // run for 10 seconds
    ...
    std::ofstream log_file("uart_tx.log");
    if (!log_file.is_open()) {
        std::cerr << "Error opening log file!" << std::endl;
        return;
    }

    // Get and write start timestamp
    auto start_time = system_clock::now();
    std::time_t start_c = system_clock::to_time_t(start_time);
    log_file << "=== START: " << std::put_time(std::localtime(&start_c), "%Y-%m-%d %H:%M:%S") << " ===" << std::endl;
    ...
    while (system_clock::now() < end_time) {
        ...
        uart.speak(buffer, 2);
        log_file << "uart tx char: " << counter << std::endl;
        counter++;

        std::this_thread::sleep_for(interval);
    }

    // Get and write end timestamp
    auto finish_time = system_clock::now();
    std::time_t finish_c = system_clock::to_time_t(finish_time);
    log_file << "=== END: " << std::put_time(std::localtime(&finish_c), "%Y-%m-%d %H:%M:%S") << " ===" << std::endl;
