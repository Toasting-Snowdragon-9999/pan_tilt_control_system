              ...
              FP32 output_min, FP32 output_max)
{
    ...
    INT32S max_pid_output_pan   = PID_Compute(pid, MAX_DEGREE_PAN, 0.0);
    INT32S max_pid_output_tilt  = PID_Compute(pid, MAX_DEGREE_TILT, 0.0);

    // Increment values for mapping
    INT32S pan_step_increment = max_pid_output_pan / MAX_MAP_STEP;
    INT32S tilt_step_increment = max_pid_output_pan / MAX_MAP_STEP;
...
void vPidControllerTask(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (xQueueReceive(xUartRxQueue, &in, pdMS_TO_TICKS(100)) == pdTRUE)
        { // macro for delay???
            uart_print("\r\n PID in: \r\n");
            uart_putc(in);
            measured_value = in;
        ...
        if (xQueueSend(xUartTxQueue, &output, pdMS_TO_TICKS(200)) == pdTRUE)
        { // macro for delay???
            uart_print("\r\n PID out: \r\n");
            // uart_putc(output);
        }
