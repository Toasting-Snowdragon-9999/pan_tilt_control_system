...
INT32S PID_compute(struct PID_controller *PID_controller, INT8S vision_reference, INT8S enc_measured_val)
{
    ...
    // Derivative led
    INT32S D = -(PID_controller->alpha / PID_controller->beta) * PID_controller->prev_derivative
               + (PID_controller->gamma / PID_controller->beta) * (error - PID_controller->prev_error);
...
void unpack_spi_frame(struct PID_controller *PID_controller, uint16_t message){ //From FPGA to TIVA (And vision to TIVA)
...
uint16_t create_frame(struct PID_controller *PID_controller){ //From TIVA to FPGA
...
void PID_task(void *pvParameter)
{
    ...
    for(EVER){
        ...
        if(uart_new_flag){
            ...
            PID_controller->combined_ref_pan = PID_controller->vision_ref_pan + PID_controller->enc_pan;
            PID_controller->combined_ref_tilt = PID_controller->vision_ref_tilt + PID_controller->enc_tilt;
